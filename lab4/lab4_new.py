...
# С первым писком ничего не сделано
# сделано первые 3 пункта строка 253
...
class FrameStats(IRenderable):
    ...
    def render_debug(self):
        ...
        text = "Tick duration: {0} ms\nFPS: {1}".format(int(1000 * duration),
                                                        int(1.0 / duration))

        self.game.next_frame_canvas.create_text(650, 50, text=text,
                                                font=("Arial", 20),
                                                justify=tk.RIGHT)
...
class Ball(IRenderable, IIntersectable, IClickable):
    def __init__(self, game, position: Vector2d, radius: float):
        ...
        self.a = rnd.randint(0,10000)
        self.velocity = Vector2d(rnd.randrange(-50, 50),
                                 rnd.randrange(-50, 50))
        super().__init__(game)

    def render(self):
        c="#{:06x}".format((self.color[0]<<16)+(self.color[1]<<8)+self.color[2])
        self.a += 0.01
        cos = math.cos(self.a)
        """
        self.color[0] += round(2 * cos)
        self.color[1] -= round(3 * cos)
        self.color[2] += round(2 * cos)
"""
        self.game.next_frame_canvas.create_oval(self.position.x - self.radius,
    ...
    def render_debug(self):
        self.game.next_frame_canvas.create_oval(self.position.x - 2,
                                                self.position.y - 2,
                                                self.position.x + 2,
                                                self.position.y + 2,
                                                fill='black', width=0)
        self.game.next_frame_canvas.create_text(self.position.x,
                                                self.position.y + 15,
                                                text=str(self.uuid).split('-')[
                                                    0], fill='black')

        vel_base = self.position + self.velocity * self.radius * (
                1 / abs(self.velocity))
        self.game.next_frame_canvas.create_line(vel_base.x, vel_base.y,
                                                vel_base.x + self.velocity.x,
                                                vel_base.y + self.velocity.y,
                                                width='3')

    def contains(self, point: Vector2d):
        return self.radius > abs(
            complex(self.position.x - point.x, self.position.y - point.y))
...
class Game:
    ...
    def tick(self):
        ...
        self.tick_stamp = time.time()
        """
        все изменения находятся в этом if
        """
        if not self.pause:
            x = self.g.x
            self.g.x = self.g.x * self.cos - self.g.y * self.sin
            self.g.y = x * self.sin + self.g.y * self.cos
            for v in self.objects.values():
                if isinstance(v, Ball):
                    v.velocity += self.g
            for v in self.objects.values():
                if isinstance(v, Ball):
                    for vv in self.objects.values():
                        if vv != v:
                            if isinstance(vv, Ball):
                                p = vv.position - v.position
                                if abs(p) <= v.radius + vv.radius:
                                    v.velocity.x = -v.velocity.x
                                    v.velocity.y = -v.velocity.y
                                    vv.velocity.x = -vv.velocity.x
                                    vv.velocity.y = -vv.velocity.y
                                    v.position += v.velocity * 0.02
                                    vv.position += vv.velocity * 0.02
    ...
    def clicked(self, event):
        ...
        for key in self.objects:
            obj = self.objects[key]
            if isinstance(obj, IIntersectable) and isinstance(obj,
                                                              IClickable) and obj.contains(
                point):
                obj.clicked()
